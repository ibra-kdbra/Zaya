...
class AppState {
    ...
    static NAVIGATOR_TABS = ['thumbs', 'outline', 'search', 'text'];
    static PANEL_TABS = ['Document', 'Notes', 'Media', 'Settings'];

    // Interface languages. The document's own reading direction (`isRTL`) is a separate setting.
    ...
    constructor() {
        this.state = {
            ...
            currentPdfType: 'url',
            currentPdfName: '',
            // A local file's size in bytes: with its name it makes the document's key, so two
            // files that share a name keep their own pages, notes and recognised text.
            currentPdfSize: 0,
            lastPage: 1,
            theme: 'default',
            // Interface language ('en' | 'ar'); empty until the first visit has been resolved
            language: '',
            mediaVolume: 50,
            mediaLoop: false,
            mediaMode: 'youtube',
            ...
            navigatorOpen: false,
            navigatorTab: 'thumbs',   // 'thumbs' | 'outline' | 'search' | 'text'
            panelOpen: false,
    ...
    loadFromStorage() {
        try {
            ...
            Object.keys(stored).forEach(key => {
                ...
                } else if (key === 'navigatorTab' || key === 'panelTab' || key === 'language'
                    || key === 'hardCover') {
            ...
            const storedSize = parseInt(localStorage.getItem('lastOpenedPDFSize'), 10);
            if (storedPdf && storedPdf.trim() !== '') {
                if (storedType === 'local' || storedPdf.startsWith('blob:')) {
                    ...
                    this.state.currentPdfSize = Number.isFinite(storedSize) && storedSize > 0 ? storedSize : 0;
                } else {
    ...
    persistToStorage() {
        try {
            ...
            if (this.state.language) localStorage.setItem('language', this.state.language);
            ...
            localStorage.setItem('lastOpenedPDFSize', this.state.currentPdfType === 'local'
                ? String(this.state.currentPdfSize || 0)
                : '0');
        } catch (error) {
    ...
    /**
     * Update the open document. `pdfSize` is the size in bytes of a file picked from disk; it is
     * part of that document's key, so it is kept when a later call (the loader's own, which knows
     * only the URL and the name) leaves it out.
     */
    updatePdfContext(pdfUrl, pdfType, pdfName, pdfSize) {
        ...
        const size = Number(pdfSize);
        if (this.state.currentPdfType !== 'local') this.state.currentPdfSize = 0;
        else if (Number.isFinite(size) && size > 0) this.state.currentPdfSize = size;
        else if (this.state.currentPdfName !== prevState.currentPdfName) this.state.currentPdfSize = 0;
    ...
    // Interface language ('en' | 'ar'). ZayaI18n owns the switch; this only records the choice.
    setLanguage(language) {
        if (!AppState.LANGUAGES.includes(language)) return;
        this.set({ language });
    }
    ...
    // Which section of the Navigator is showing ('thumbs' | 'outline' | 'search' | 'text')
    setNavigatorTab(tab) {
