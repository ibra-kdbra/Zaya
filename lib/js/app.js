...
const MODULES = new Set([
    'lib/js/core/engine.js',
    'lib/js/features/navigator/panes.js',
...
async function loadApplication() {
    try {
        // 1. Vendored libraries: Toastify, and nothing else. The engine brings its own three.js
        //    and pdf.js as ES modules (vendor/three, vendor/pdfjs) and imports them itself.
        await loadInOrder([
            'vendor/js/toastify.min.js'
        ]);
        ...
        await loadInOrder([
            ...
            // url-options first: the language preset it parses decides which dictionary applies
            ...
            // The engine is an ES module; core/engine.js is the one line that imports it and
            // leaves the constructor on window for the facade to find.
            'lib/js/core/engine.js',
        ...
        await loadInOrder([
            ...
            'lib/js/ui/share-box.js',
            'lib/js/features/navigator/panes.js',
            'lib/js/features/documents/recent.js',
            'lib/js/features/controls/custom-controls.js',
            'lib/js/features/text/text-pane.js',
            'lib/js/features/print/print.js',
            'lib/js/features/media/media.js',
            'lib/js/features/settings/language.js',
            'lib/js/features/themes/manager.js',
            'lib/js/features/themes/selector.js',
...
function initializeApp() {
    ...
    // Set before the event, so a listener that arrives late can still tell it has already fired.
    window.ZAYA_INITIALIZED = true;
    document.dispatchEvent(new CustomEvent('zaya:init', { detail: { version: ZAYA_VERSION } }));
}

// The last resort: the loader itself failed, so the dictionaries may never have arrived. This one
// message stays in English rather than risking a blank screen.
