...
window.ZayaTextPane = (function () {
    ...
    function pageSection(pdfPage, paragraphs) {
        ...
        head.appendChild(element("h3", "text-page-title", "Page " + pdfPage));
        const copy = element("button", "text-page-copy");
        copy.type = "button";
        copy.title = "Copy page text";
        const copyIcon = element("i", "fas fa-copy");
        copyIcon.setAttribute("aria-hidden", "true");
        copy.append(copyIcon, element("span", null, "Copy page text"));
        ...
        if (!paragraphs.length) {
            section.appendChild(element("p", "text-page-blank", "This page carries no text."));
        }
        return section;
    }
    ...
    function copyPage(pdfPage, button) {
        ...
        writeClipboard(text).then((ok) => {
            ...
            setTimeout(() => { label.textContent = "Copy page text"; }, 1600);
        });
    }
    ...
    function render(force) {
        ...
        if (!ctrl) {
            renderedKey = "";
            showState("fas fa-align-left", "The text of the pages you are reading will appear here.");
            return;
        }
        ...
        if (texts.every((t) => t === null)) {
            showState("fas fa-hourglass-half", "Reading page text…");
            requestIndex();
            return;
        }
        ...
        if (ctrl.isComplete && stats.withText === 0) {
            showState("fas fa-image",
                "This document has no text: its pages are images. They can be recognised on this device from the Search tab.",
                { label: "Open Search", onClick: () => { if (window.ZayaNavigator) window.ZayaNavigator.setTab("search"); } });
            return;
        }
    ...
    function buildActionBar() {
        ...
        bar.append(
            make("fas fa-quote-left", "Add as note", () => addNote()),
            make("fas fa-copy", "Copy", () => {
                const text = selectionText;
                writeClipboard(text).then(() => hideBar());
            }),
            make("fas fa-search", "Search", () => {
    ...
    function addNote() {
        ...
        hideBar();
        if (!text) return;
        if (window.ZayaQuotes && window.ZayaQuotes.add) window.ZayaQuotes.add(text, page);
    }
