...
export class PdfTextSearch {
  ...
  /**
   * A page's text as the lines it was laid out in, so a reader can be shown it as prose.
   * The index stores one flat string per page plus the box of every run; a change of vertical
   * position starts a new line, and the caller decides where the paragraph breaks go from the
   * gaps between them.
   *
   * @param {number} pageNum 1-based PDF page number
   * @returns {null | Array<{text: string, y: number, height: number}>}
   *   `null` while the page is still to be indexed, an empty array when it carries no text.
   */
  getPageText(pageNum) {
    const entry = this.pages[pageNum];
    if (!entry) return null;
    if (!entry.raw) return [];
    if (!entry.spans || !entry.spans.length) return [{ text: entry.raw, y: 0, height: 0 }];
    const lines = [];
    let current = null;
    for (const s of entry.spans) {
      const h = Math.max(s.height || Math.abs(s.transform[3]) || 0, 1);
      const y = s.transform[5];
      if (current && Math.abs(y - current.y) < h * 0.5) {
        current.end = s.end;
        current.height = Math.max(current.height, h);
        continue;
      }
      if (current) lines.push(current);
      current = { start: s.start, end: s.end, y, height: h };
    }
    if (current) lines.push(current);
    return lines.map((l) => ({ text: entry.raw.slice(l.start, l.end).trim(), y: l.y, height: l.height }))
      .filter((l) => l.text);
  }
  ...
  /**
   * Pages (1-based) that have been indexed, have nothing searchable and have not been through
   * recognition yet. A recognised page that turned out blank (a cover, a plate) is not offered again.
   */
  pagesNeedingText() {
    const out = [];
    for (let p = 1; p <= this.numPages; p++) {
      const e = this.pages[p];
      if (e && !e.ocr && !this.hasUsableText(p)) out.push(p);
    }
    return out;
  }
  ...
  setPageFromLines(pageNum, lines) {
    ...
    // Anything showing this page's text (the Text pane) repaints when recognition lands.
    if (typeof document !== "undefined" && document.dispatchEvent) {
      document.dispatchEvent(new CustomEvent("zaya:pageTextChanged", { detail: { page: pageNum } }));
    }
  }
  ...
  index(onProgress) {
    ...
    const extractOne = async (pageNum) => {
      ...
      try {
        ...
        // Recognised text (from the store) may have arrived while this page was being read;
        // it is the better entry and has already been counted.
        if (self.pages[pageNum]) return;
