...
window.ZayaNavigator = (function () {
    ...
    /**
     * Drop every adopted panel: called before another document is loaded. A query belongs to the
     * document it was typed against, so the search box, its results and the marks painted from it
     * go with the panels rather than following the reader into the next book.
     */
    function reset() {
        const host = bodyEl();
        const fb = book();
        if (fb) fb.setSearchHighlight("");
        document.querySelectorAll(".df-search-input").forEach((input) => { input.value = ""; });
        document.querySelectorAll(".df-search-results, .df-search-status").forEach((box) => box.replaceChildren());
    ...
    /*
     * The engine (engine/ui/ui.js) and the scrim both call `close()` to mean "the reader tapped
