...
zayaWhenReady(function () {
    ...
    function updateVisibilityMode() {
        ...
        if (label) label.textContent = t(barAlwaysShown() ? "settings.alwaysVisible" : "settings.autoHide");
    }
    ...
    document.addEventListener("mousemove", function (e) {
        ...
        const isHoveringSidebar = !!closestFrom(e.target, '.nav-pane');
    ...
    window.addEventListener("wheel", (e) => {
        if (!closestFrom(e.target, '.nav-pane')) return;
    ...
    /*
     * The page-mode override is remembered twice: once as the reader's general preference, and
     * once with the document that was open when it was chosen, so a book that reads better as a
     * single page opens that way again while everything else keeps the general choice.
     */
    function rememberPageMode(mode) {
        writePref(PAGE_MODE_KEY, mode);
        const key = window.ZayaCurrentDocKey ? window.ZayaCurrentDocKey() : "";
        if (key && window.ZayaDocPrefs) window.ZayaDocPrefs.set(key, { pageMode: mode });
    }
    ...
    window.ZayaBookPrefs = {
        ...
        /** The page mode this document was last read in, when it has one of its own. */
        pageModeFor(key) {
            if (!key || !window.ZayaDocPrefs) return Promise.resolve(null);
            return window.ZayaDocPrefs.get(key)
                .then((rec) => (rec && (rec.pageMode === "single" || rec.pageMode === "double") ? rec.pageMode : null))
                .catch(() => null);
        },
        ...
        applyTo(book) {
            ...
            apply(this.pageMode());
            // The document's own choice, once it has been read back, wins over the general one.
            const key = window.ZayaCurrentDocKey ? window.ZayaCurrentDocKey() : "";
            this.pageModeFor(key).then(apply);
            syncUI();
        }
    };
    ...
    function syncUI() {
        ...
        setLabel("menuFullscreenBtn", t(isFullscreen ? "more.exitFullscreen" : "more.fullscreen"));
    }
    ...
    // The bar's own labels are rewritten by syncUI; the switch value has no other trigger.
    document.addEventListener("zaya:languageChanged", () => { updateVisibilityMode(); syncUI(); });
    // A new document: the page number and the counts follow it in the same event cycle rather
    // than waiting for the next turn of the loop below.
    document.addEventListener("zaya:pdfLoaded", () => { syncUI(); setTimeout(syncUI, 0); });
...
/*
 * Navigator: one left drawer holding the Pages, Outline, Search and Text panes as tabs.
 *
 * All four are the application's own -- `features/navigator/panes.js` builds the first three
 * from what the engine facade offers as data, and `features/text/text-pane.js` the fourth --
 * and all four are built straight into `#navigatorBody`. The drawer's whole job is to say which
 * one is showing, by keeping `nav-pane-visible` on exactly one of them.
 */
window.ZayaNavigator = (function () {
    const TABS = ["thumbs", "outline", "search", "text"];
    // The first three come from the book (through the facade); "text" is the Text pane's own.
    const BOOK_TABS = ["thumbs", "outline", "search"];
    const SELECTOR = { thumbs: "#navPaneThumbs", outline: "#navPaneOutline", search: "#navPaneSearch", text: "#navPaneText" };
    ...
    const EMPTY = {
        ...
        outline: { icon: "fas fa-stream", key: "nav.empty.outline" },
        search: { icon: "fas fa-magnifying-glass-minus", key: "nav.empty.search" },
        text: { icon: "fas fa-align-left", key: "nav.empty.text" }
    };
    ...
    /**
     * The panes build themselves into the drawer, so there is nothing left to move. The name
     * survives because the drawer still has to make sure each pane carries the tab wiring, and
     * because `ZayaNavigator.adopt()` is published.
     */
    function adopt() {
        const host = bodyEl();
        if (!host) return;
    ...
    /**
     * Another document is being loaded. The marks painted from the last query go with it, and the
     * Text pane starts again; the panes themselves stay until the new document can replace them
     * (`ZayaPanes.adopt`), so the drawer never stands empty while a book is opening.
     */
    function reset() {
        const fb = book();
        if (fb) fb.setSearchHighlight("");
    ...
    function updateEmptyState() {
        ...
        else if (activeTab === "outline" && !p.querySelector(".nav-outline-item")) state = EMPTY.outline;
        else if (activeTab === "thumbs" && !p.querySelector(".nav-page")) state = EMPTY.thumbs;
    ...
    function paint() {
        TABS.forEach((tab) => {
            const p = pane(tab);
            if (p) p.classList.toggle("nav-pane-visible", isOpen && tab === activeTab);
    ...
    function setTab(tab, opts) {
        ...
        if (tab === "search") {
            const fb = book();
            const input = fb ? fb.searchInput() : null;
    ...
    /*
     * The scrim and an outside tap both call `close()` to mean "the reader tapped outside me".
     * That is only a dismissal while the drawer overlays the book: a docked drawer
    ...
    function init() {
        ...
        // An open driven from outside the drawer (`?search=`, the Text pane's "search this")
        // toggles the class on a pane directly; the drawer follows it.
        const nb = bodyEl();
        if (nb) {
            new MutationObserver((records) => {
                records.forEach((r) => {
                    ...
                    if (!t) return;
                    const visible = r.target.classList.contains("nav-pane-visible");
                    if (visible && !(isOpen && activeTab === t)) open(t, { focusSearch: false });
                    else if (!visible && isOpen && activeTab === t) {
                        // A docked drawer is layout: a "tap the book to dismiss" must not take it
                        // away, so re-assert instead of closing.
        ...
        // Capture phase, so a pane that stops keydown propagation cannot swallow Escape.
        ...
        // The empty states are the drawer's own words, so they follow a language switch.
        document.addEventListener("zaya:languageChanged", updateEmptyState);
    ...
    return {
        open, close: requestClose, toggle, setTab, adopt, reset,
        isOpen: () => isOpen,
        activeTab: () => activeTab,
        /* A pane was rebuilt under the drawer: give it the tab wiring and show the right one. */
        refresh: () => { adopt(); paint(); }
    };
})();
...
(function () {
    function update() {
        ...
        if (!box || !host) return;
        const ready = !!host.querySelector("canvas, .zn-page") || host.textContent.trim().length > 0;
        box.hidden = ready;
    }
