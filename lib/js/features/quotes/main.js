...
window.ZayaQuotes = {
  add(text, pageNumber, done) {
    ...
    if (clean.length < 3) {
      toast('Select at least three characters to keep as a note.', '#ef4444', 3000);
      if (done) done(false);
      return;
    }
    if (!window.dbInitialized) {
      toast('Notes are not ready yet. Please try again in a moment.', '#f59e0b', 4000);
      if (done) done(false);
      return;
    }
    const pdfKey = currentQuoteKey();
    const pdfName = window.appState.get('currentPdfName') || 'Local PDF';
    const page = Number(pageNumber) || window.appState.get('lastPage') || null;
