...
export function displayQuotes(quotes) {
  ...
  if (quotes.length === 0) {
    quoteList.appendChild(element('div', 'no-quotes', 'No quotes saved yet. Add your first quote!'));
    return;
  }
  ...
  quotes.forEach((quote) => {
    ...
    meta.appendChild(element('span', 'quote-page-info', quote.pageNumber ? ` • p.${Number(quote.pageNumber)}` : ''));
    content.appendChild(meta);

    const actions = element('div', 'quote-actions');
    actions.appendChild(iconButton('panel-button editQuoteBtn', 'fas fa-edit', 'Edit Quote', quote.id));
    actions.appendChild(iconButton('panel-button deleteQuoteBtn', 'fas fa-trash', 'Delete Quote', quote.id));
...
export function displayQuotesInModal(quotes) {
  ...
  if (quotes.length === 0) {
    ...
    icon.setAttribute('aria-hidden', 'true');
    empty.append(icon, element('h3', null, 'No quotes for this PDF yet'),
      element('p', null, 'Add quotes while reading this PDF to see them here.'));
  ...
  headerLeft.append(element('h3', null, '📝 Quotes from Current PDF'),
    element('span', 'quotes-count', `${quotes.length} quote${quotes.length !== 1 ? 's' : ''}`));
  ...
  exportBtn.title = 'Export PDF Quotes';
  exportBtn.setAttribute('aria-label', 'Export PDF Quotes');
  ...
  quotes.forEach((quote) => {
    ...
    meta.appendChild(element('span', 'modal-quote-timestamp', `📅 ${timestamp} at ${time}`));
    meta.appendChild(element('span', 'modal-quote-pdf', `📄 ${quote.pdfName || 'Unknown PDF'}`));
    if (quote.pageNumber) meta.appendChild(element('span', 'modal-quote-page', `📍 Page ${Number(quote.pageNumber)}`));
    content.appendChild(meta);

    const actions = element('div', 'modal-quote-actions');
    actions.appendChild(iconButton(themed('panel-button editQuoteBtn'), 'fas fa-edit', 'Edit Quote', quote.id));
    actions.appendChild(iconButton(themed('panel-button deleteQuoteBtn'), 'fas fa-trash', 'Delete Quote', quote.id));
