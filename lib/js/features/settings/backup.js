...
const PREF_KEYS = ['theme', 'isRTL', 'mediaVolume', 'mediaLoop'];
...
function toast(text, color) {
  if (window.Toastify) {
    window.Toastify({ text, duration: 4000, gravity: 'bottom', position: 'right', backgroundColor: color }).showToast();
  }
}
  ...
  return new Promise((resolve) => {
    getAllQuotes((quotes) => {
      getSettings((settings) => {
        ...
        toast(`Exported ${n} quote${n === 1 ? '' : 's'} and your preferences`, '#22c55e');
        resolve(payload);
      });
    });
  });
    ...
    reader.onload = () => {
      ...
      getAllQuotes((existing) => {
        ...
        const finish = () => {
          ...
          toast(`Imported ${incoming.length} new quote${incoming.length === 1 ? '' : 's'}`, '#22c55e');
