 ...
 *
 * Format 2 adds `recent`, `ocr` and `documents` (what each document remembers of its own: its page
 * mode and the soundtrack it was last read with). Files themselves are never part of a backup: a
 * restored file entry is metadata, so it reads as "not kept" until the reader picks the file again.
 * Recognised text can be large, so it is left out (with a note in the file) beyond OCR_MAX_BYTES.
 * Format 1 files are still accepted; they simply carry none of those sections.
 *
 * Everything is filed under the document key (see lib/js/utils/pageMemory.js). A backup written
 * before local files were keyed by name and size carries their bare filenames; those records move
 * to the new key by themselves the first time that file is opened from disk again.
 */
...
/** What each document remembers of its own: page mode, and the soundtrack it was read with. */
function collectDocuments() {
  if (!window.ZayaDB) return Promise.resolve([]);
  return window.ZayaDB.getAll('settings')
    .then((rows) => (rows || []).filter((r) => r && typeof r.id === 'string' && r.id.indexOf(DOC_PREFIX) === 0))
    .catch(() => []);
}

/** Put those records back, so a restored document opens the way it was left. */
function applyDocuments(documents) {
  if (!Array.isArray(documents) || !documents.length || !window.ZayaDB) return Promise.resolve(0);
  const clean = documents
    .filter((r) => r && typeof r.id === 'string' && r.id.indexOf(DOC_PREFIX) === 0 && r.id.length < 2100)
    .slice(0, 500)
    .map((r) => {
      const record = { id: r.id };
      if (r.pageMode === 'single' || r.pageMode === 'double') record.pageMode = r.pageMode;
      if (r.media && typeof r.media === 'object') {
        record.media = {
          mode: r.media.mode === 'audio' ? 'audio' : 'youtube',
          youtube: typeof r.media.youtube === 'string' ? r.media.youtube.slice(0, 2048) : ''
        };
      }
      return record;
    });
  return clean.reduce((chain, r) => chain.then(() => window.ZayaDB.put('settings', r)), Promise.resolve())
  ...
  return new Promise((resolve) => {
    getAllQuotes((quotes) => {
      getSettings((settings) => {
        Promise.all([collectOcr(), collectDocuments()]).then(([{ ocr, notes }, documents]) => {
          const payload = {
            format: FORMAT,
