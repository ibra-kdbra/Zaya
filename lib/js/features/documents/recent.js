...
(function () {
    ...
    function open(entry) {
        ...
        if (entry.type === 'url') {
            ...
            if (!safe) { toast(t('recent.badLink'), '#ef4444'); return; }
        ...
        (store ? store.getFile(entry.key) : Promise.resolve(null)).then((rec) => {
            if (rec && rec.blob) {
                // The File carries the real filename: with its size it is the document's key.
                const name = entry.name || rec.label || entry.key;
                const file = rec.blob instanceof File ? rec.blob : new File([rec.blob], name, { type: 'application/pdf' });
                window.ZayaDocuments.openLocalFile(file, { skipSave: true });
                return;
            }
            toast(t('recent.gone', { name: entry.name }));
    ...
    function ensureStorageUi() {
        ...
        free.textContent = t('recent.freeUp');
        free.title = t('recent.freeUpTitle');
        ...
        yes.addEventListener('click', () => {
            ...
            (store ? store.freeUpSpace() : Promise.resolve({ files: 0, bytes: 0, ocrPages: 0 })).then((result) => {
                ...
                if (!result.files && !result.ocrPages) toast(t('recent.nothingToFree'));
                else toast(freed ? t('recent.freed', { size: freed }) : t('recent.freedSome'));
                return render();
            });
        });
    ...
    function refreshUsage() {
        ...
        return window.ZayaDB.estimate().then((e) => {
            ...
            line.hidden = false;
            line.textContent = t('recent.usage', { size: formatSize(e.usage) || '0 KB' });
        }).catch(() => {});
    }
    ...
    function buildItem(entry, currentKey) {
        ...
        btn.title = isCurrent ? t('recent.current')
            : t(entry.type === 'url' ? 'recent.openLink' : 'recent.reopenFile');
        ...
        if (entry.type === 'url') {
            ...
            state.textContent = t(kept ? 'recent.kept' : 'recent.notKept');
            meta.append(state);
        ...
        remove.title = t(kept ? 'recent.removeKept' : 'recent.remove');
        remove.setAttribute('aria-label',
            t(kept ? 'recent.removeLabelKept' : 'recent.removeLabel', { name: name.textContent }));
    ...
    function init() {
        ...
        // Every line in this group is prose; the whole group is rebuilt in the new language.
        document.addEventListener('zaya:languageChanged', () => {
            storageBox = null;
            const box = group();
            const existing = box ? box.querySelector('.storage-box') : null;
