...
(function () {
    ...
    async function renderPage(doc, pageNumber) {
        ...
        const fb = book();
        if (fb) {
            if (includeMarks) fb.drawSearchHighlights(ctx, viewport, pageNumber);
            // A watermark or any other registered painter prints whether or not the marks do.
            if (typeof fb.runPainters === "function") fb.runPainters(ctx, viewport, pageNumber, "print");
        }
