...
(function () {
    ...
    function parseRange(text, total) {
        ...
        if (!raw) return { pages: [], error: "Enter the pages to print, for example 3-7, 10." };
        const parts = raw.split(",");
        const pages = [];
        for (const part of parts) {
            const token = part.trim();
            if (!token) return { pages: [], error: "Remove the empty entry between the commas." };
            ...
            if (span) {
                ...
                if (from < 1 || to < 1 || from > total || to > total) {
                    return { pages: [], error: `Pages run from 1 to ${total}.` };
                }
                const step = from <= to ? 1 : -1;
                for (let n = from; step > 0 ? n <= to : n >= to; n += step) pages.push(n);
            } else if (single) {
                const n = parseInt(single[1], 10);
                if (n < 1 || n > total) return { pages: [], error: `Pages run from 1 to ${total}.` };
                pages.push(n);
            } else {
    ...
    function selection() {
        const total = pageCount();
        if (!total) return { pages: [], error: "No document is open." };
    ...
    function sync() {
        ...
        if (errorLine) {
            errorLine.textContent = showError ? result.error : "";
        ...
        if (warning) {
            if (count > LARGE_RANGE) {
                warning.textContent = confirmPending
                    ? `Ready to prepare ${count} pages. Choose Print again to confirm.`
                    : `${count} pages is a lot to prepare; it may take a while.`;
        ...
        if (label && !busy) label.textContent = confirmPending ? `Print ${count} pages` : "Print";
    }
    ...
    async function renderAndPrint(pages) {
        ...
        for (let i = 0; i < pages.length; i++) {
            if (cancelled) { releaseSheet(); busy = false; setProgress(""); sync(); return false; }
        ...
        busy = false;
        if (cancelled || !sheet.childElementCount) { releaseSheet(); setProgress(""); sync(); return false; }
    ...
    function resolveRange(range) {
        ...
        if (Array.isArray(range)) {
            ...
            return { pages, error: pages.length ? null : `Pages run from 1 to ${total}.` };
        }
