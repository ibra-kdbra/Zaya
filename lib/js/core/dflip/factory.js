...
import { hasWebgl, log } from './utils.js';
...
export class FlipBook extends PreviewObject {
  constructor(container, source, options) {
    ...
    // The renderer can be pinned for testing and for browsers whose WebGL is unusable:
    // `window.ZAYA_RENDER_MODE = "css" | "webgl"` wins, otherwise `?render=css|2d|webgl|3d`.
    const useWebgl = forcedRenderMode() === "css" ? false
      : forcedRenderMode() === "webgl" ? true
      : (options.webgl == true && hasWebgl == true);
    self.webgl = useWebgl;
    self.renderMode = useWebgl ? "webgl" : "css";
...
/**
 * A renderer pinned by the page or the URL, or null to let WebGL support decide.
 * `window.ZAYA_RENDER_MODE` is checked first, then `?render=`; both accept
 * "css"/"2d" and "webgl"/"3d".
 * @returns {"css"|"webgl"|null}
 */
function forcedRenderMode() {
  const normalise = (value) => {
    const v = String(value == null ? "" : value).toLowerCase();
    if (v === "css" || v === "2d" || v === "html") return "css";
    if (v === "webgl" || v === "3d") return "webgl";
    return null;
  };
  const pinned = normalise(window.ZAYA_RENDER_MODE);
  if (pinned) return pinned;
  try {
    return normalise(new URLSearchParams(window.location.search).get("render"));
  } catch (e) {
    return null;
  }
}
