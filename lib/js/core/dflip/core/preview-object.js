...
import { log, limitAt, getBasePage, hasFullscreenEnabled, isRTLMode, distOrigin, prefersSinglePage } from '../utils.js';
...
export class PreviewObject {
  ...
  resize() {
    ...
    // Automatic page mode follows the viewport (portrait phones get one page, landscape tablets two)
    // unless the user or the ?mode= option chose explicitly.
    if (options.pageMode == null && !self.userPageMode && target.pageCount > 2) {
      const wantSingle = prefersSinglePage(containerWidth, jQuery(window).height());
      const desired = wantSingle ? PAGE_MODE.SINGLE : PAGE_MODE.DOUBLE;
      if (target.pageMode !== desired) {
        target.pageMode = desired;
        if (target.singlePageMode == SINGLE_PAGE_MODE.BOOKLET && target.reset) target.reset();
        if (this.ui && this.ui.pageMode) {
          this.ui.pageMode.toggleClass(options.icons["doublepage"], wantSingle);
        }
      }
    }
    const isSinglePage = target.pageMode == PAGE_MODE.SINGLE;
    // On narrow screens the side menu overlays the book (see custom-ui.css) instead of pushing it.
    // Width of the Navigator drawer when it pushes the book (medium widths); phones overlay instead.
    // When the drawers are docked (>=1200px, body.dock-*) the container itself has already been
    // narrowed by its margins, so the stage must not shift a second time -- it centres in what is left.
    const drawerEl = document.getElementById("navigatorDrawer");
    const drawerWidth = drawerEl && drawerEl.offsetWidth ? drawerEl.offsetWidth : 220;
    const isDocked = document.body.classList.contains("dock-left") || document.body.classList.contains("dock-right");
    const sideMenuWidth = !isDocked && container.hasClass("df-sidemenu-open") && containerWidth > 640 ? drawerWidth : 0;
  ...
  setPageMode(isSingle, fromUser) {
    if (fromUser) this.userPageMode = true; // an explicit choice wins over the viewport heuristic
    if (isSingle == true) {
