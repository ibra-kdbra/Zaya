  ...
  initSearch() {
    ...
    const render = (query) => {
      ...
      if (!hits.length) {
        ...
        if (st.withText === 0) {
          status.text("This document has no text layer (its pages are images), so it cannot be searched.");
        } else if (st.unreadable >= Math.max(1, st.withText / 2)) {
          status.text("The text in this document is stored as glyph codes without a Unicode mapping, so it cannot be searched.");
