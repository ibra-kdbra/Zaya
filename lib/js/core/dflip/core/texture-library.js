...
export class TextureLibrary {
  ...
  initSearch() {
    ...
    const ocrLang = $("<div>").addClass("df-ocr-langs").attr({ role: "radiogroup", "aria-label": "Language of the text to recognise" });
    const ocrLangButtons = LANG_CHOICES.map((c) => $("<button>").attr({ type: "button", role: "radio", "data-lang": c.id, "aria-checked": String(c.id === ocrLangValue) }).addClass("df-ocr-langbtn").text(c.label));
    ocrLangButtons.forEach((b) => ocrLang.append(b));
    const ocrHint = $("<p>").addClass("df-ocr-hint").text("One language is about twice as fast as two.");
    ...
    const runOcr = async () => {
      ...
      const report = (busy) => {
        loadingShown = false;
        const inFlight = busy.size ? `page${busy.size === 1 ? "" : "s"} ${Array.from(busy).sort((a, b) => a - b).join(", ")}` : "";
        ocrProgress.text(`Recognising ${inFlight}… ${done} of ${total} done${eta()}`);
      };
      const busy = new Set();
      const stopped = () => ocrCancel || self.searchController !== controller;
      const lane = async () => {
      ...
      try {
        ...
        if (done) ocrProgress.text(ocrCancel ? `Stopped after ${plural(done, "page")}. Recognised pages are kept.` : `Recognised ${plural(done, "page")} in ${took}.`);
