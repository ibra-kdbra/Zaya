...
import { OcrEngine, OcrStore, LANG_CHOICES, preferredLanguage, rememberLanguage, languageLabel } from '../../../features/search/ocr.js';
...
export class TextureLibrary {
  ...
  initSearch() {
    ...
    const ocrLang = $("<select>").addClass("df-ocr-lang").attr("aria-label", "Language of the text to recognise");
    LANG_CHOICES.forEach((c) => ocrLang.append($("<option>").attr("value", c.id).text(c.label)));
    ocrLang.val(preferredLanguage());
    const ocrRun = $("<button>").attr("type", "button").addClass("ui-btn ui-btn-primary df-ocr-run").text("Recognise text");
    const ocrProgress = $("<p>").addClass("df-ocr-progress").attr("aria-live", "polite");
    ocrRow.append(ocrLang).append(ocrRun);
    ocrBox.append(ocrText).append(ocrRow).append(ocrProgress);
    form.append(input);
    ...
    const docKey = window.ZayaCurrentDocKey ? window.ZayaCurrentDocKey() : "";
    const plural = (n, word) => `${n} ${word}${n === 1 ? "" : "s"}`;
    let ocrRunning = false;
    let ocrCancel = false;

    const bumpHighlights = () => {
      if (!self.highlightQuery) return;
      self.highlightToken = String(Date.now() % 1e9);
      self.refreshVisiblePages();
    };

    const updateOcrBox = () => {
      if (self.searchController !== controller) return;
      ...
      if (!need.length) {
        ...
        ocrText.text(`${plural(st.ocr, "page")} recognised on this device. Search covers them.`);
        ocrRow.hide();
        return;
      }
      ocrBox.removeAttr("hidden");
      ...
      ocrText.text(need.length === st.pages
        ? "This document has no text layer: its pages are images. Recognise them on this device to search them. Nothing is uploaded."
        : `${need.length} of ${st.pages} pages have no text layer. They can be recognised on this device; nothing is uploaded.`);
      ocrRun.text(need.length === st.pages ? "Recognise text" : `Recognise ${plural(need.length, "page")}`);
    };

    // Recognised text from an earlier visit joins the index straight away.
    OcrStore.load(docKey).then((cached) => {
      if (!cached.size || self.searchController !== controller) return;
    ...
    const runOcr = async () => {
      if (ocrRunning || self.searchController !== controller) return;
      const pages = controller.pagesNeedingText();
      if (!pages.length) return;
      const lang = ocrLang.val();
      rememberLanguage(lang);
      ocrRunning = true;
      ocrCancel = false;
      ocrRun.text("Stop");
      ocrLang.prop("disabled", true);
      ocrProgress.text("Loading the recognition engine…");
      const engine = new OcrEngine(lang, (info) => {
        if (!ocrRunning) return;
        if (info.stage.indexOf("loading") === 0 || info.stage.indexOf("initializ") === 0) {
          ocrProgress.text(`Loading ${languageLabel(lang)} language data…`);
        }
      });
      self.ocrEngine = engine;
      // Start at the page being read, then continue through the rest of the book.
      const current = Math.max(1, Math.min(self.pageCount || 1, (self.targetObject && self.targetObject._activePage) || 1));
      const ordered = pages.filter((p) => p >= current).concat(pages.filter((p) => p < current));
      let done = 0;
      try {
        for (const p of ordered) {
          if (ocrCancel || self.searchController !== controller) break;
          ocrProgress.text(`Recognising page ${p}… ${done} of ${pages.length} done`);
          const page = await self.pdfDocument.getPage(p);
        ...
        if (!ocrCancel) {
          ...
          ocrProgress.text("Text recognition failed: " + (err && err.message ? err.message : "unknown error"));
        }
      } finally {
        engine.terminate();
        if (self.ocrEngine === engine) self.ocrEngine = null;
        ocrRunning = false;
        ocrLang.prop("disabled", false);
        if (done) ocrProgress.text(ocrCancel ? `Stopped after ${plural(done, "page")}. Recognised pages are kept.` : `Recognised ${plural(done, "page")}.`);
    ...
    ocrRun.on("click", (e) => {
      ...
      if (ocrRunning) {
        ocrCancel = true;
        ocrProgress.text("Stopping…");
        if (self.ocrEngine) self.ocrEngine.terminate();
      } else {
        runOcr();
      }
    });
    ocrLang.on("click keydown", (e) => e.stopPropagation());

    // Index as soon as the panel is shown, so the offer to recognise pages appears without a query.
    let indexKicked = false;
    const kickIndex = () => {
      if (indexKicked || self.searchController !== controller) return;
      indexKicked = true;
      controller.index().then(updateOcrBox);
    };
    new MutationObserver(() => { if (container.hasClass("df-sidemenu-visible")) kickIndex(); })
      .observe(container[0], { attributes: true, attributeFilter: ["class"] });
    ...
    const render = (query) => {
      ...
      if (!hits.length) {
        if (!done) { status.text("Searching…"); return; }
        const st = self.searchController.stats();
        if (st.withText === 0) {
          status.text("No searchable text yet: the pages are images.");
        } else if (st.unreadable >= Math.max(1, st.withText / 2)) {
          status.text("No matches. The text is stored as glyph codes without a Unicode mapping.");
        } else if (st.withText < st.pages / 2) {
          status.text(`No matches. Only ${st.withText} of ${st.pages} pages carry searchable text.`);
        } else {
