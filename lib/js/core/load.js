...
function failOrFallback(kind, pdfUrl, rtlMode) {
    ...
    Toastify({ text: "That document could not be opened. Showing the default document.", duration: 4000, gravity: "bottom", position: "right", backgroundColor: "#f59e0b" }).showToast();
...
function showDocumentError(kind, source) {
    ...
    h.textContent = 'This document could not be opened';
    const p = document.createElement('p');
    p.textContent = kind === 'cors'
        ? 'The site hosting it does not allow other pages to load it. Download the PDF and open it from this device, or ask the host to enable cross-origin access.'
        : (kind === 'invalid' ? 'The link is not a valid PDF address.' : 'The link did not respond or the file is not a readable PDF. Check the link or try another file.');
    ...
    other.type = 'button'; other.className = 'primary'; other.textContent = 'Open another document';
...
function showThemedLocalFileToast(filename) {
    ...
    line.textContent = `Last read: "${filename}". Please re-select it to continue.`;
