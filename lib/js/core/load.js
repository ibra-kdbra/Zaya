...
/*
 * `options.text` is the engine's own labels, already translated. The engine draws no chrome of
 * its own any more, so the block mostly goes unread; it stays because the contract
 * (`docs/engine-api.md` §1) has it, and an engine that did draw its own controls would need it.
 */
...
function failOrFallback(kind, pdfUrl, rtlMode) {
    ...
    loadFlipbook(DEFAULT_PDF_URL, rtlMode, 1, window.ZayaDocKey(DEFAULT_PDF_URL, 'url'));
}
...
async function loadFlipbook(pdfUrl, rtlMode, page, pdfId) {
    ...
    var options = {
        ...
        onReady: function(book) {
            ...
            if (window.ZayaLocalDocs && pdfUrl !== DEFAULT_PDF_URL) {
                ...
                // The entry is keyed by the document key; the name is what the reader sees.
                window.ZayaLocalDocs.touch({
                    key: isLocal ? (pdfId || '') : (pdfId || pdfUrl),
                    type: isLocal ? 'local' : 'url',
                    name: isLocal ? (window.appState.get('currentPdfName') || 'Local PDF') : pdfUrl,
                    size: isLocal ? (window.appState.get('currentPdfSize') || 0) : 0
                });
            }
    ...
    setTimeout(() => {
        ...
        if (pdfUrl === window.appState.get('currentPdf')) {
            ...
            const hasDFlipContent = !!(host && host.querySelector(".zn-stage canvas, .zn-page"));
...
function showDocumentError(kind, source) {
    ...
    retry.addEventListener('click', () => {
        hideDocumentError();
        if (source) loadFlipbook(source, window.appState.get('isRTL'), 1, window.ZayaDocKey(source, 'url')).catch(() => {});
    });
...
function updatePdfContext(pdfUrl, pdfId) {
    ...
    if (pdfUrl.startsWith('blob:')) {
        pdfType = 'local';
        // `pdfId` is the document key (name and size); the name the reader sees is kept in state
...
/*
 * Open the first document. Deferred to the end of the current task, as the ready callback it
 * replaces was, so that every script the loader queued has run before the first book is asked for.
 */
function openFirstDocument() {
    ...
    var storedSize = window.appState.get('currentPdfSize');
    ...
    if (pdfFromUrl) {
        ...
        pdfId = window.ZayaDocKey(pdfFromUrl, 'url');
    } else if (storedType === 'local' && storedName) {
        // The blob URL died with the previous page load. If a copy of the file was kept it is
        // reopened from IndexedDB; otherwise show the default and ask for a re-pick. The copy is
        // looked for under the document key first and under the bare filename after it, so a
        // profile written before keys carried the size still finds its file.
        const store = window.ZayaLocalDocs;
        const storedKey = window.ZayaDocKey('', 'local', storedName, storedSize);
        const restore = store
            ? store.getFile(storedKey).then((rec) => (rec || (storedKey === storedName ? null : store.getFile(storedName))))
            : Promise.resolve(null);
        restore.then(function (rec) {
            if (rec && rec.blob) {
                const file = rec.blob;
                const fileUrl = URL.createObjectURL(file);
                const size = Number(file.size) || storedSize || 0;
                window.appState.updatePdfContext(fileUrl, 'local', storedName, size);
                const docKey = store && file instanceof File ? store.keyOf(file) : storedKey;
                const ready = store && file instanceof File ? store.adopt(file).catch(() => docKey) : Promise.resolve(docKey);
                return ready.then((key) => window.getLastPage(key).catch(() => null).then(function (storedPage) {
                    return loadFlipbook(fileUrl, window.appState.get('isRTL'), (!isNaN(pageFromUrl) && pageFromUrl) || storedPage || 1, key);
                }));
            }
            showThemedLocalFileToast(storedName);
            window.appState.updatePdfContext(DEFAULT_PDF_URL, 'url', 'Default PDF');
            const defaultKey = window.ZayaDocKey(DEFAULT_PDF_URL, 'url');
            return window.getLastPage(defaultKey).catch(() => null).then(function (storedPage) {
                return loadFlipbook(DEFAULT_PDF_URL, window.appState.get('isRTL'), storedPage || 1, defaultKey);
            });
        ...
        pdfId = window.ZayaDocKey(pdfToLoad, 'url');
    }
...
/*
 * The first book waits for zaya:init, which app.js dispatches once every queued script has run and
 * the presets in the address have been applied. Waiting for the DOM alone was not enough: ?rtl=1
 * sets the reading direction from that same event, and a timeout could win the race, so the book
 * opened left-to-right and the arrow keys turned the wrong way. The inner timeout lets the other
 * listeners of zaya:init -- url-options among them -- finish before anything is opened.
 */
function openWhenReady() {
    setTimeout(() => {
        // A plugin may hold the first book back (a password gate, a share link that has to be
        // checked first): it calls preventDefault on this event and detail.resume() when the reader
        // is allowed in. resume() opens the book once; further calls do nothing.
        let opened = false;
        const resume = () => {
            if (opened) return;
            opened = true;
        ...
        const gate = new CustomEvent('zaya:beforeOpen', { cancelable: true, detail: { resume } });
        if (document.dispatchEvent(gate)) resume();
    }, 0);
}

if (window.ZAYA_INITIALIZED) openWhenReady();
else document.addEventListener('zaya:init', openWhenReady, { once: true });
