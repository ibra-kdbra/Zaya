 ...
 *     and after them whatever painters the application registers (`ZayaBook.painters`).
 *
...
(function () {
    ...
    /*
     * Painters: what the application wants drawn into a page as it is rendered, after the search
     * marks. A watermark, say. They live in the page texture, so they show in whichever renderer
     * is running, and the print feature runs them too, so a printed sheet carries them as well.
     * Each is `(ctx, viewport, pdfPage, { purpose }) => void` with `purpose` "screen" or "print".
     */
    const painters = new Set();
    function runPainters(ctx, viewport, pdfPage, purpose) {
        if (!ctx || !viewport || !painters.size) return false;
        let drew = false;
        painters.forEach((paint) => {
            ctx.save();
            try { if (paint(ctx, viewport, Number(pdfPage) || 1, { purpose: purpose || "screen" }) !== false) drew = true; }
            catch (e) { console.warn("ZayaBook: a painter failed", e); }
            ctx.restore();
        });
        return drew;
    }
    ...
    function Handle(instance, created) {
        ...
        /** Run the registered painters for one PDF page on any 2D context (the print feature's own canvases). */
        self.runPainters = (ctx, viewport, pdfPage, purpose) => runPainters(ctx, viewport, pdfPage, purpose);
    ...
    function engineOptions(options, handleRef) {
        ...
        out.paintPage = function (ctx, viewport, pdfPage) {
            ...
            runPainters(ctx, viewport, pdfPage, "screen");
        };
    ...
    window.ZayaBook = {
        ...
        /**
         * Painters drawn into every page after the search marks; see `runPainters` above. Adding
         * or removing one repaints the pages on screen, so a watermark switched on over an open
         * book appears at once.
         */
        painters: {
            add(paint) {
                if (typeof paint !== "function" || painters.has(paint)) return false;
                painters.add(paint);
                const b = current();
                if (b && b.isReady()) b.refreshVisiblePages();
                return true;
            },
            remove(paint) {
                if (!painters.delete(paint)) return false;
                const b = current();
                if (b && b.isReady()) b.refreshVisiblePages();
                return true;
            },
            has(paint) { return painters.has(paint); },
            get size() { return painters.size; }
        }
    };
})();
