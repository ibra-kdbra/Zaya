...
(function () {
    ...
    onClick("loadPdfUrlBtn", function () {
        ...
        if (!rawUrl) {
            toast("Please enter a PDF URL.", "#ef4444");
            return;
        }
        ...
        if (!window.ValidationUtils.isValidPdfUrl(newPdfUrl)) {
            toast("URL doesn't appear to point to a PDF file.", "#f59e0b");
    ...
    function syncDirectionLabel() {
        ...
        if (label) label.textContent = isRTL ? "Right to left" : "Left to right";
        const btn = el("toggleDirectionBtn");
        if (btn) {
            btn.setAttribute("aria-pressed", isRTL ? "true" : "false");
            btn.setAttribute("title", isRTL ? "Switch to left-to-right reading" : "Switch to right-to-left reading");
        }
    }
