$(document).ready(function () {
    ...
    function setPanelOpen(open, from) {
        ...
        if (open) {
            ...
            if (window.matchMedia("(max-width: 768px)").matches && window.ZayaNavigator) window.ZayaNavigator.close();
            setPanelTab(storedPanelTab(), false);
            if (window.matchMedia("(max-width: 768px)").matches && window.ZayaA11y) {
    ...
    $(document).on("keydown", "#unifiedPanel [data-panel-tab]", function (e) {
        ...
        let next = -1;
        if (e.key === "ArrowRight") next = (idx + 1) % PANEL_TABS.length;
        else if (e.key === "ArrowLeft") next = (idx + PANEL_TABS.length - 1) % PANEL_TABS.length;
    ...
    $("#loadPdfUrlBtn").click(function () {
        ...
        window.getLastPage(newPdfUrl).catch(() => null).then(function(storedPage) {
    ...
    $("#pdfFile").change(function () {
        ...
        window.getLastPage(fileName).catch(() => null).then(function(storedPage) {
