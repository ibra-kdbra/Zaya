 ...
 * controls that live inside it. Plain DOM, like the rest of the reader.
 */
(function () {
     ...
     *
     * The document key is the file's name and its size, so two different files of the same name
     * stay apart; anything the same file was filed under by its bare name moves across first.
     */
    function openLocalFile(file, opts) {
        const fileName = file.name;
        const fileUrl = URL.createObjectURL(file);
        const docs = window.ZayaLocalDocs;
        const key = docs ? docs.keyOf(file) : fileName;
        const ready = docs ? docs.adopt(file).catch(() => key) : Promise.resolve(key);
        if (docs && !(opts && opts.skipSave)) {
            ready.then(() => docs.saveFile(file)).then((kept) => {
                if (!kept && file.size > docs.MAX_FILE_BYTES) {
                    Toastify({ text: t("doc.tooLargeToKeep"), duration: 5000, gravity: "bottom", position: "right" }).showToast();
                }
            });
        }
        return ready
            .then((docKey) => window.getLastPage(docKey).catch(() => null).then(function (storedPage) {
                /*
                 * The document changes at the moment it starts opening, not at the moment it was
                 * picked: adopting the file and reading back its page take a turn or two, and
                 * everything that follows a change of document -- the drawers, the notes list,
                 * the page counts -- would otherwise be looking at the new name over the old book.
                 */
                window.appState.updatePdfContext(fileUrl, 'local', fileName, file.size);
                return loadFlipbook(fileUrl, window.appState.get('isRTL'), storedPage || 1, docKey)
    ...
    function openRemoteUrl(newPdfUrl) {
        ...
        // The key is the link with its fragment and tracking parameters taken off.
        const key = window.ZayaDocKey ? window.ZayaDocKey(newPdfUrl, 'url', hostname) : newPdfUrl;
        return window.getLastPage(key).catch(() => null).then(function (storedPage) {
            window.appState.updatePdfContext(newPdfUrl, 'url', hostname);
            return loadFlipbook(newPdfUrl, window.appState.get('isRTL'), storedPage || 1, key)
    ...
    /*
     * The gesture (shift and a drag, or the right button) is not discoverable on its own: nobody
     * finds a held modifier by accident. This is the same thing where a reader can see it, and it
     * is also how they set an angle exactly rather than by feel.
     */
    function tiltControls() {
        return { range: document.getElementById("tiltRange"), value: document.getElementById("tiltValue") };
    }

    function showTilt(degrees) {
        const { range, value } = tiltControls();
        const shown = Math.round(degrees);
        if (range && Number(range.value) !== shown) range.value = String(shown);
        if (value) value.textContent = `${shown}\u00b0`;
    }

    function syncTilt() {
        const book = window.ZayaBook && window.ZayaBook.current;
        showTilt(book && book.tilt ? book.tilt.degrees : 90);
    }

    document.addEventListener("input", function (event) {
        const range = closestFrom(event.target, "#tiltRange");
        if (!range) return;
        const book = window.ZayaBook && window.ZayaBook.current;
        const degrees = Number(range.value) || 90;
        if (book && book.setTilt) book.setTilt(degrees);
        showTilt(degrees);
    });

    document.addEventListener("click", function (event) {
        if (!closestFrom(event.target, "#tiltResetBtn")) return;
        const book = window.ZayaBook && window.ZayaBook.current;
        if (book && book.resetTilt) book.resetTilt();
        showTilt(90);
    });

    // The gesture and the slider are the same setting, so each follows the other.
    document.addEventListener("zaya-engine:tiltChanged", syncTilt);
    document.addEventListener("zaya:pdfLoaded", syncTilt);
    syncTilt();
    ...
    document.addEventListener("keydown", function (event) {
        ...
        /*
         * Arrow keys turn pages. They are read as directions on the screen rather than as
         * "forward" and "back": the right arrow moves the book to the right, which is the next
         * page in a left-to-right document and the previous one in a right-to-left one. The
         * engine takes pointer input only, so this is the whole keyboard story for page turns.
         */
        if (event.key === "ArrowRight" || event.key === "ArrowLeft") {
            if (event.ctrlKey || event.metaKey || event.altKey || event.shiftKey) return;
            // Inside a drawer or a dialog an arrow key belongs to whatever has focus there.
            if (active && active.closest && active.closest("#navigatorDrawer, #unifiedPanel, .modal-container, .theme-modal")) return;
            const book = window.ZayaBook ? window.ZayaBook.current : null;
            if (!book || !book.isReady()) return;
            const rightwards = event.key === "ArrowRight";
            const forwards = book.direction === "rtl" ? !rightwards : rightwards;
            event.preventDefault();
            if (forwards) book.next(); else book.prev();
            return;
        }
