...
- **Search Hits on the Page**: matches are painted onto the rendered page itself (works in the 3D and 2D renderers), not only listed in the panel; marks update as you type and disappear when the panel closes.
- **Mobile Layout**: page mode now follows the viewport instead of the user-agent string: portrait phones get one page filling the width, landscape tablets and desktops get the spread; an explicit choice (menu or `?mode=`) still wins. The version label moves out from under the control bar on small screens.
- **Visual Refresh**: a quieter, warmer default theme (ink-dark tinted neutrals, one brass accent, neutral shadows), self-hosted IBM Plex Sans/Mono replacing Inter, flat 44px controls, no lift-on-hover or tinted glows, no interface text under 12px, visible keyboard focus, reduced-motion support. Rationale and tokens are in `DESIGN.md`; the other 53 themes keep their palettes with neutralised shadows.
